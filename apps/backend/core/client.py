...
def _detect_worktree_parent(project_dir: Path) -> Path | None:
    """If project_dir is inside an Auto-Claude worktree, return the parent project root."""
    markers = (
        "/.auto-claude/worktrees/tasks/",
        "/.auto-claude/github/pr/worktrees/",
        "/.worktrees/",
    )
    posix = str(project_dir.resolve()).replace("\\", "/")
    for m in markers:
        if m in posix:
            return Path(posix.rsplit(m, 1)[0])
    return None


def _landlock_available() -> bool:
    """Probe whether landlock_create_ruleset(0) is supported by the running kernel."""
    try:
        import ctypes
        libc = ctypes.CDLL("libc.so.6", use_errno=True)
        class _RA(ctypes.Structure):
            _fields_ = [("h_fs", ctypes.c_uint64),
                        ("h_net", ctypes.c_uint64),
                        ("scoped", ctypes.c_uint64)]
        # Try with ALL_FS bits, stripping on EINVAL — same approach as the wrapper.
        handled = (1 << 15) - 1
        while handled:
            attr = _RA(handled, 0, 0)
...
def _generate_landlock_wrapper(
    project_dir: Path, real_cli_path: str | None
) -> Path | None:
    """
    Generate a shell wrapper that invokes the real Claude CLI under landlock-exec.

    Returns the wrapper path, or None if landlock is not usable on this host.
    The wrapper is written to <project_dir>/.auto-claude-landlock-wrapper.sh.
    """
    # Tough OS check: must be Linux, not WSL-on-Windows-host edge cases. We rely
    # on sys.platform (returns "linux" on real Linux and WSL2) AND verify the
    # landlock syscalls actually work on the running kernel.
    if not sys.platform.startswith("linux"):
        logger.warning(
            f"AUTO_CLAUDE_LANDLOCK=1 set, but sys.platform={sys.platform!r}; "
            "landlock is Linux-only — skipping wrapper."
        )
        return None
    if not _landlock_available():
        logger.warning(
            "AUTO_CLAUDE_LANDLOCK=1 set, but landlock syscalls not available on "
            "this kernel — skipping wrapper. Verify with: "
            "uname -r (need >= 5.13) and that no `lsm=` boot param disables landlock."
        )
        return None

    landlock_exec = Path(__file__).resolve().parent.parent / "tools" / "landlock_exec.py"
    if not landlock_exec.exists():
        logger.warning(f"AUTO_CLAUDE_LANDLOCK=1 set, but {landlock_exec} not found.")
        return None

    # Resolve real CLI: explicit override > $CLAUDE_CLI_PATH > shutil.which("claude")
    real = real_cli_path or os.environ.get("CLAUDE_CLI_PATH") or shutil.which("claude")
    if not real or not Path(real).exists():
        logger.warning(
            "AUTO_CLAUDE_LANDLOCK=1 set, but cannot resolve real claude CLI path "
            "(set CLAUDE_CLI_PATH or ensure `claude` is on PATH) — skipping wrapper."
        )
        return None

    parent = _detect_worktree_parent(project_dir)
    extra_rw = []
    if parent:
        # Grant access to the parent's .auto-claude/ subtree only — spec files
        # live there. The rest of the parent project root stays blocked.
        spec_root = parent / ".auto-claude"
        if spec_root.exists():
            extra_rw.append(str(spec_root.resolve()))

    wrapper = project_dir / ".auto-claude-landlock-wrapper.sh"
    rw_args = [f'--rw "{project_dir.resolve()}"'] + [f'--rw "{p}"' for p in extra_rw]
    script = (
        "#!/bin/sh\n"
        "# Auto-generated by Auto-Claude (AUTO_CLAUDE_LANDLOCK=1). Do not edit.\n"
        f'exec python3 "{landlock_exec}" \\\n'
        "    --preset standard \\\n"
        f"    {' '.join(rw_args)} \\\n"
        "    --best-effort \\\n"
        f'    -- "{real}" "$@"\n'
...
def create_client(
    ...
    # Optional: Wrap the CLI in landlock-exec so the agent process and all its
    # descendants are kernel-restricted to project_dir + the parent's .auto-claude
    # spec directory. Opt-in via AUTO_CLAUDE_LANDLOCK=1; Linux-only.
    if os.environ.get("AUTO_CLAUDE_LANDLOCK") == "1" and sys.platform.startswith("linux"):
        wrapper = _generate_landlock_wrapper(
            project_dir=project_dir,
            real_cli_path=options_kwargs.get("cli_path"),
        )
        if wrapper:
            options_kwargs["cli_path"] = str(wrapper)
            logger.info(f"Landlock sandbox enabled; CLI wrapped via {wrapper}")
