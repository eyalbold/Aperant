#!/usr/bin/env bash
# start-docker-macos.sh
# macOS variant: reads the Claude OAuth token from the login Keychain
# (service "Claude Code-credentials") instead of ~/.claude/.credentials.json,
# then launches Auto-Claude via Docker Compose.
# Use --reauth to run `claude /login` and refresh the token first.
# Usage: ./scripts/start-docker-macos.sh [--project-path <path>] [--extra-mount-path <path>] [--build] [--reauth] [--dont-remove]
...
echo "================================================="
echo "  Auto-Claude Docker Launcher (macOS / Keychain)"
echo "================================================="
echo ""

# ── Step 1: Read token from macOS Keychain ────────────────────────────────────
if [[ -z "${CLAUDE_CODE_OAUTH_TOKEN:-}" ]]; then
  ...
  if $REAUTH || [[ -z "$CREDS_JSON" ]]; then
    echo "[1/2] Running 'claude /login' for account '$KC_ACCOUNT'..."
  ...
  if [[ -z "$CREDS_JSON" ]]; then
    echo "ERROR: No Keychain entry found for service '$KC_SERVICE' (account '$KC_ACCOUNT')."
    echo "       Run 'claude /login' and approve Keychain access, then retry."
    exit 1
  fi

  TOKEN="$(printf '%s' "$CREDS_JSON" | python3 -c "import json,sys; print(json.load(sys.stdin)['claudeAiOauth']['accessToken'])" 2>/dev/null || true)"

  if [[ -z "$TOKEN" ]]; then
    TOKEN="$(printf '%s' "$CREDS_JSON" | jq -r '.claudeAiOauth.accessToken // empty' 2>/dev/null || true)"
  fi

  if [[ -z "$TOKEN" ]]; then
    echo "ERROR: Could not extract accessToken from Keychain payload. Run with --reauth."
    exit 1
  fi

  echo "  ✓  Token loaded from Keychain (account '$KC_ACCOUNT'): ${TOKEN:0:16}..."
  echo ""
